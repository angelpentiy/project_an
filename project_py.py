...
class Main(tk.Frame):
    def __init__(self, root):
        ...
        self.db = db
def init_main(self):
     toolbar = tk.Frame(bg='#585d71', bd=2)
     toolbar = tk.Frame(bg='#585d71', bd=2)
     self.add_image = tk.PhotoImage(file='./img/add.png')
     btn_open_dialog = tk.Button( toolbar, bg='#585d71', bd=0,
     image=self.add_image, command=self.open_dialog)
     btn_open_dialog.pack(side=tk.RIGHT)
     self.update_image = tk.PhotoImage(file='./img/update.png')
     btn_edit_dialog = tk.Button(toolbar, bg='#585d71', bd=0, 
     image=self.update_image, command=self.open_update_dialog)
     btn_edit_dialog.pack(side=tk.RIGHT)
     self.delete_image = tk.PhotoImage(file='./img/delete.png')
     btn_delete = tk.Button(toolbar, bg='#585d71', bd=0, 
     image=self.delete_image, command=self.delete_records)
     btn_delete.pack(side=tk.RUGHT)
     self.search_image = tk.PhotoImage(file='./img/search.png')
     btn_search = tk.Button(toolbar, bg='#585d71', bd=0,
     ...
     btn_refresh = tk.Button(toolbar, bg='#585d71', bd=0, 
    ...
    height=45, show='headings')
     ...
     self.tree.heading("tel", text='№ Телефона')
    ...
    for row in self.db.c.fetchall()]
     ...
     def init_child(self):
          ...
          self.entry_email.place(x=280, y=150)
     ...
     label_search.place(x=200, y=50)
     self.entry_search = ttk.Entry(self)
     self.entry_search.place(x=200, y=50, width=250)
     ...
     class DB:
      def __init__(self):
        ...
        self.c.execute('''CREATE TABLE IF NOT EXISTS db (id integer primary key, name text, tel text, email text)''')
        self.conn.commit()  
